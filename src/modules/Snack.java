...
public class Snack {
    ...
    private int getSnackKarma(int snackid) {
        String selectStatement = "SELECT votecount FROM snacks WHERE id = ? ";
        int votecount = -999;
        try {
            prepStmt = db.prepareStatement(selectStatement);
            prepStmt.setInt(1, snackid);
    ...
    public String voteSnack(String msg, String user) {
        ...
        if (msg.matches("[\\\\+-] \\d+")) {
            ...
            if (votes.userHasVoted(id, user)) {
                ...
                int karma = getSnackKarma(id);
                return "You have voted " + vote + " for snack " + id + ". [k:" + karma + "]";
            }
        } else {
